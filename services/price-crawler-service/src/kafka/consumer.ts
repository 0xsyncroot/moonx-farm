...
export class KafkaConsumer {
  ...
  async connect(topics: string[]): Promise<void> {
    this.kafka = new Kafka({
      clientId:
        configs.kafka.consumerConfig.clientId || "price-crawler-consumer",
  ...
  async onMessage(
    handler: (topic: string, payload: KafkaJobPayload) => Promise<void>
  ): Promise<void> {
    ...
    await this.consumer.run({
      eachMessage: async ({ topic, message }) => {
        if (message.value) {
          ...
          console.log(
            `[KafkaConsumer] Received message from ${topic}:`,
            payload
          );
